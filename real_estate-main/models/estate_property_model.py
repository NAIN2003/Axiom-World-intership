...
class RealEstate(models.Model):
    ...
    def cancel_property(self):
        if self.status != 'new':
            raise exceptions.UserError('Only properties with status "New" can be canceled.')
        self.status = 'canceled'

    def mark_as_sold(self):
        if self.status != 'offer_received':
            raise exceptions.UserError('Only properties with status "Offer Received" can be marked as sold.')
        self.status = 'sold'